...
slap_time = cell(num_geckos,num_trials);
stroke_vel_vert = cell(num_geckos,num_trials);
stroke_vel_hor = cell(num_geckos,num_trials);
stroke_time = cell(num_geckos,num_trials);
stroke_dist_vert = cell(num_geckos,num_trials);
stroke_dist_hor = cell(num_geckos,num_trials);
bslap_vel = cell(num_geckos,num_trials);
bslap_time = cell(num_geckos,num_trials);
bstroke_vel_vert = cell(num_geckos,num_trials);
bstroke_vel_hor = cell(num_geckos,num_trials);
bstroke_time = cell(num_geckos,num_trials);
bstroke_dist_vert = cell(num_geckos,num_trials);
bstroke_dist_hor = cell(num_geckos,num_trials);
...
avg_slaptime = zeros(num_geckos,num_trials);
avg_strokevel_vert = zeros(num_geckos,num_trials);
avg_strokevel_hor = zeros(num_geckos,num_trials);
avg_stroketime = zeros(num_geckos,num_trials);
avg_strokedist_vert = zeros(num_geckos,num_trials);
avg_strokedist_hor = zeros(num_geckos,num_trials);
avg_bslapvel = zeros(num_geckos,num_trials);
avg_bslaptime = zeros(num_geckos,num_trials);
avg_bstrokevel_vert = zeros(num_geckos,num_trials);
avg_bstrokevel_hor = zeros(num_geckos,num_trials);
avg_bstroketime = zeros(num_geckos,num_trials);
avg_bstrokedist_vert = zeros(num_geckos,num_trials);
avg_bstrokedist_hor = zeros(num_geckos,num_trials);
avg_period = zeros(num_geckos,num_trials);
fslap_imp = zeros(num_geckos,num_trials);
fslap_force = zeros(num_geckos,num_trials);
bslap_imp = zeros(num_geckos,num_trials);
bslap_force = zeros(num_geckos,num_trials);
fstr_imp_vert = zeros(num_geckos,num_trials);
fstr_force_vert = zeros(num_geckos,num_trials);
fstr_imp_hor = zeros(num_geckos,num_trials);
fstr_force_hor = zeros(num_geckos,num_trials);
fstr_drag_hor = zeros(num_geckos,num_trials);
bstr_imp_vert = zeros(num_geckos,num_trials);
bstr_force_vert = zeros(num_geckos,num_trials);
bstr_imp_hor = zeros(num_geckos,num_trials);
bstr_force_hor = zeros(num_geckos,num_trials);
bstr_drag_hor = zeros(num_geckos,num_trials);
 		...
 		if isempty(gecko_swim(which_gecko,which_trial).Head) ~= 1
			...
			if gecko_swim(which_gecko,which_trial).F_slapstroke(1,1) > gecko_swim(which_gecko,which_trial).F_recoverydown(1,1)
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).F_slapstroke(:,1)),length(gecko_swim(which_gecko,which_trial).F_recoverydown(:,1)))
                    ...
                    slap_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_slapstroke(i,1) - gecko_swim(which_gecko,which_trial).F_recoverydown(i,1))/500;
                end
			else
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).F_slapstroke(:,1))-1,length(gecko_swim(which_gecko,which_trial).F_recoverydown(:,1)))
                    ...
                    slap_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_slapstroke(i+1,1) - gecko_swim(which_gecko,which_trial).F_recoverydown(i,1))/500;
                end
			end
			avg_slapvel(which_gecko,which_trial) = mean(slap_vel{which_gecko,which_trial});
            avg_slaptime(which_gecko,which_trial) = mean(slap_time{which_gecko,which_trial});
			...
			if gecko_swim(which_gecko,which_trial).F_recoveryup(1,1) > gecko_swim(which_gecko,which_trial).F_slapstroke(1,1)
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).F_recoveryup(:,1)),length(gecko_swim(which_gecko,which_trial).F_slapstroke(:,1)))
                    ...
                    stroke_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i,1) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,1))/500;
                    stroke_dist_vert{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i,3) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,3));
                    stroke_dist_hor{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i,2) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,2));
                end
			else
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).F_recoveryup(:,1))-1,length(gecko_swim(which_gecko,which_trial).F_slapstroke(:,1)))
                    ...
                    stroke_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i+1,1) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,1))/500;
                    stroke_dist_vert{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i+1,3) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,3));
               		stroke_dist_hor{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).F_recoveryup(i+1,2) - gecko_swim(which_gecko,which_trial).F_slapstroke(i,2));
                end
			end
            ...
            avg_stroketime(which_gecko,which_trial) = mean(stroke_time{which_gecko,which_trial});
            avg_strokedist_vert(which_gecko,which_trial) = mean(stroke_dist_vert{which_gecko,which_trial});
            avg_strokedist_hor(which_gecko,which_trial) = mean(stroke_dist_hor{which_gecko,which_trial});
			...
			if gecko_swim(which_gecko,which_trial).B_recoveryup(1,1) > gecko_swim(which_gecko,which_trial).B_slapstroke(1,1)
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).B_recoveryup(:,1)),length(gecko_swim(which_gecko,which_trial).B_slapstroke(:,1)))
                    ...
                    bstroke_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_recoveryup(i,1) - gecko_swim(which_gecko,which_trial).B_slapstroke(i,1))/500;
					bstroke_dist_vert{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_recoveryup(i,3) - gecko_swim(which_gecko,which_trial).B_slapstroke(i,3));
               		bstroke_dist_hor{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_recoveryup(i,2) - gecko_swim(which_gecko,which_trial).B_slapstroke(i,2));
                end
			else
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).B_recoveryup(:,1))-1,length(gecko_swim(which_gecko,which_trial).B_slapstroke(:,1)))
                    ...
                    bstroke_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_recoveryup(i+1,1) - gecko_swim(which_gecko,which_trial).B_slapstroke(i,1))/500;
                end
			end
            ...
            avg_bstroketime(which_gecko,which_trial) = mean(bstroke_time{which_gecko,which_trial});
			avg_bstrokedist_vert(which_gecko,which_trial) = mean(bstroke_dist_vert{which_gecko,which_trial});
			avg_bstrokedist_hor(which_gecko,which_trial) = mean(bstroke_dist_hor{which_gecko,which_trial});
			...
			if gecko_swim(which_gecko,which_trial).B_slapstroke(1,1) > gecko_swim(which_gecko,which_trial).B_recoverydown(1,1)
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).B_slapstroke(:,1)),length(gecko_swim(which_gecko,which_trial).B_recoverydown(:,1)))
                    ...
                    bslap_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_slapstroke(i,1) - gecko_swim(which_gecko,which_trial).B_recoverydown(i,1))/500;
                end
			else
				for i = 1:min(length(gecko_swim(which_gecko,which_trial).B_slapstroke(:,1))-1,length(gecko_swim(which_gecko,which_trial).B_recoveryup(:,1)))
                    ...
                    bslap_time{which_gecko,which_trial}(end+1,:) = abs(gecko_swim(which_gecko,which_trial).B_slapstroke(i+1,1) - gecko_swim(which_gecko,which_trial).B_recoverydown(i,1))/500;
                end
			end
			avg_bslapvel(which_gecko,which_trial) = mean(bslap_vel{which_gecko,which_trial});
			avg_bslaptime(which_gecko,which_trial) = mean(bslap_time{which_gecko,which_trial});
			...
			fslap_force(which_gecko,which_trial) = fslap_imp(which_gecko,which_trial)/avg_slaptime(which_gecko,which_trial);
			...
			fstr_force_vert(which_gecko,which_trial) = fstr_imp_vert(which_gecko,which_trial)/avg_stroketime(which_gecko,which_trial);
            ...
            fstr_force_hor(which_gecko,which_trial) = fstr_imp_hor(which_gecko,which_trial)/avg_stroketime(which_gecko,which_trial);
            fstr_drag_hor(which_gecko,which_trial) = 999.97*sqrt(1.004*10^-6*len_fleg(which_gecko)/1000)*sqrt((avg_strokevel_hor(which_gecko,which_trial)/1000)^3) + 999.97*(avg_strokevel_hor(which_gecko,which_trial)/1000)^2*len_fleg(which_gecko)/1000;
			...
			bslap_force(which_gecko,which_trial) = bslap_imp(which_gecko,which_trial)/avg_bslaptime(which_gecko,which_trial);
			...
			bstr_force_vert(which_gecko,which_trial) = bstr_imp_vert(which_gecko,which_trial)/avg_bstroketime(which_gecko,which_trial);
            ...
            bstr_force_hor(which_gecko,which_trial) = bstr_imp_hor(which_gecko,which_trial)/avg_bstroketime(which_gecko,which_trial);
            bstr_drag_hor(which_gecko,which_trial) = 999.97*sqrt(1.004*10^-6*len_bleg(which_gecko)/1000)*sqrt((mean(bstroke_vel_hor{which_gecko,which_trial})/1000)^3)+ 999.97*(mean(bstroke_vel_hor{which_gecko,which_trial})/1000)^2*len_bleg(which_gecko)/1000;
...
dlmwrite('avg_stride.dat',avg_swing,'\t')
...
dlmwrite('avg_fwdstrokedistvert.dat',avg_strokedist_vert,'\t')
dlmwrite('avg_fwdstrokedisthor.dat',avg_strokedist_hor,'\t')
...
dlmwrite('avg_backstrokedistvert.dat',avg_bstrokedist_vert,'\t')
dlmwrite('avg_backstrokedisthor.dat',avg_bstrokedist_hor,'\t')
...
for which_gecko = 1:num_geckos
	for which_trial = 1:num_trials
		if avg_vel(which_gecko,which_trial) ~= 0 
            ...
            compiled_data(i,13) = avg_strokedist_vert(which_gecko,which_trial);
            compiled_data(i,14) = avg_strokedist_hor(which_gecko,which_trial);
			compiled_data(i,15) = avg_bstrokedist_vert(which_gecko,which_trial);
            compiled_data(i,16) = avg_bstrokedist_hor(which_gecko,which_trial);
			compiled_data(i,17) = avg_stance(which_gecko,which_trial);
			compiled_data(i,18) = avg_swing(which_gecko,which_trial);
			compiled_data(i,19) = avg_period(which_gecko,which_trial);
			compiled_data(i,20) = fslap_imp(which_gecko,which_trial);
            compiled_data(i,21) = fslap_force(which_gecko,which_trial);
			compiled_data(i,22) = fstr_imp_vert(which_gecko,which_trial);
            compiled_data(i,23) = fstr_force_vert(which_gecko,which_trial);
            compiled_data(i,24) = fstr_imp_hor(which_gecko,which_trial);
            compiled_data(i,25) = fstr_force_hor(which_gecko,which_trial);
            compiled_data(i,26) = fstr_drag_hor(which_gecko,which_trial);
			compiled_data(i,27) = bslap_imp(which_gecko,which_trial);
            compiled_data(i,28) = bslap_force(which_gecko,which_trial);
			compiled_data(i,29) = bstr_imp_vert(which_gecko,which_trial);
            compiled_data(i,30) = bstr_force_vert(which_gecko,which_trial);
            compiled_data(i,31) = bstr_imp_hor(which_gecko,which_trial);
            compiled_data(i,32) = bstr_force_hor(which_gecko,which_trial);
            compiled_data(i,33) = bstr_drag_hor(which_gecko,which_trial);
			compiled_data(i,34) = min_imp(which_gecko,which_trial);
			compiled_data(i,35) = est_imp(which_gecko,which_trial);
			compiled_data(i,36) = percent(which_gecko,which_trial);
			i = i+1;
		end
	end
end
